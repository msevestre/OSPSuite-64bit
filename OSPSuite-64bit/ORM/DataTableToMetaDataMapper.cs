...
namespace OSPSuite_64bit.ORM
{
   ...
   public class DataTableToMetaDataMapper<T> : IDataTableToMetaDataMapper<T> where T : new()
   {
      ...
      private void checkDataTableStructure(DataTable dt)
      {
         ...
         foreach (DataColumn dataColumn in dt.Columns)
         {
            if (!allPropertyNames.Contains(dataColumn.ColumnName))
               errorList.Add(Constants.Error.MissingColumnInView(dataColumn.ColumnName));
      ...
      private class Constants
      {
         public class Error
         {
            public static string MissingColumnInView(string dataColumnColumnName)
            {
               return $"missing column named {dataColumnColumnName}";
            }
         }
      }
   }
